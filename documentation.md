...
This document explains how Smart Kanban is structured internally, how data flows, how drag‑and‑drop is wired, and how to extend the system safely. The app uses a lightweight Node.js API with a local JSON database (`db.json`), a single‑account token login, and a single‑server deploy model.
...
- On first launch, **no user exists** → the UI shows Create Account.
- `POST /signup` creates the single user (only once).
...
- `GET /auth/status` — check if a user exists (no auth)
- `POST /signup` — create the single user (no auth, only once)
...
## 16. Production (Single Command)
```
npm run start
```
This builds the frontend and serves it with the API on the same port.
